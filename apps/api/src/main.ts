...
import { reportProcessError } from "./observability.js";
...
void bootstrap();
process.on("unhandledRejection",error=>reportProcessError("process.unhandled_rejection",error));process.on("uncaughtException",error=>{reportProcessError("process.uncaught_exception",error);setTimeout(()=>process.exit(1),100).unref()});
