...
import { roleHome } from "@/lib/account";
import { isRole } from "@/lib/domain/authorization";
import { normalizeIndonesianPhone } from "@/lib/domain/phone";
import { consumeAuthRateLimit,createSession,safeReturnPath,verifyPassword } from "@/lib/security/auth";

export const dynamic="force-dynamic";

export async function POST(request:Request){
  const reqId=requestId(request);const body=await request.json().catch(()=>null)as{identifier?:string;password?:string;returnTo?:string}|null;const identifier=body?.identifier?.trim().toLowerCase()??"";
