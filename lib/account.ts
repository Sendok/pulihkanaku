...
export type Account = { id:string;email:string;fullName:string;role:Role;businessName:string|null };

export async function findAccountByEmail(email:string):Promise<Account|null>{
  const [row]=await getDb().select({id:users.id,email:users.email,fullName:users.fullName,role:users.role,status:users.status,businessName:businesses.name}).from(users).leftJoin(businesses,eq(businesses.ownerUserId,users.id)).where(eq(users.email,email.trim().toLowerCase())).limit(1);
  if(!row||row.status!=="ACTIVE"||!isRole(row.role))return null;
  return{id:row.id,email:row.email,fullName:row.fullName||row.email,role:row.role,businessName:row.businessName??null};
}
